...
|---|---|---|
| **`check-pr.yml`** <br/> *(Check PR)* | Pull request to `main` | Two jobs, `Library` and `Web`, both required to merge. `Library` runs a diff-checker that decides separately whether the tests need to run and whether a `CHANGELOG.md` entry is required, then formatting, linting (`clippy`), `cargo test` (unit and logic tests; fixture-backed snapshot tests are run locally), the WASM build, and a check of the Python bindings. `Web` builds the WASM package with `wasm-pack` and then the Vue site, so a broken frontend is caught before it reaches the docs deploy. |
| **`publish-unreleased.yml`** <br/> *(Publish Unreleased)* | Merge to `main` | Leverages concurrency groups to cancel outdated runs. Delegates the build of the WASM module and Vue site to `cargo xtask deploy-site --unreleased`, which force-pushes the assets to the `gh-pages` branch under `/unreleased`. That directory sits alongside `/v0.4` and the other tagged versions — side by side, not separate environments. |
| **`publish.yml`** <br/> *(Publish)* | Git tag on `main` (e.g., `v0.4.1`) | Verifies the tag is on the `main` branch to prevent rogue releases, then fans out: Crates.io, NPM (with `--provenance`), and a wheel matrix across Linux, macOS and Windows plus an sdist, gathered by a final job that uploads them to PyPI together. A last job deploys the versioned frontend to `gh-pages` using `cargo xtask`. |
