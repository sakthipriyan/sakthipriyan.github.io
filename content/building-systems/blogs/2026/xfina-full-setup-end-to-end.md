...
| **`deploy-beta.yml`** <br/> *(Deploy Beta)* | Merge to `main` | Leverages concurrency groups to cancel outdated runs. Delegates the build of the WASM module and Vue site to `cargo xtask deploy-site --unreleased`, which force-pushes the assets to the `gh-pages` branch under `/unreleased`. Beta means `main` and nothing else. |
