...
For years, the standard approach to processing hundreds of gigabytes of data involved heavy infrastructure. We relied on running Spark jobs and later Go based distributed system running on Kubernetes clusters. While powerful, this compute-forward deployment model comes with high maintenance overhead, constant sustenance work, and the inherent friction of node and pod bring-up times. Go platform uses storage as shuffle requiring no internode communication.
...
- **Statelessness (Mostly)**: While Lambda instances are likely reused—which requires carefully resetting global variables and local files—they fundamentally lack persistent local state.
...
Crucially, the Driver does not process the bulk of the data. It calculates what needs to be done, chunks the work into a task group (each task group contains multiple tasks) and returns a state to the Step Function.
...
- `sleep`: Pause for a configured duration, then loop back to the Driver — useful for polling or micro-batch scenarios. (Used this for pipeline jobs and external orchestrations)
...
- **Parallel Execution**: The Step Function handles the concurrency, launching up to 40 executors simultaneously to process the task groups. Within each executor, it can process multiple tasks in parallel assigned to it from the Task Group.
...
- **Data Passing**: The Driver and Executors communicate across stages via AWS S3 buckets (Read/Write/Read-Only) and the AWS SSM Parameter Store used credential secrets.
...
Lambda execution environments are likely reused across invocations. This means credentials, environment variables, local temporary files, and any global variables from a previous invocation may still be present. Your code must explicitly reset all shared state at the start of each invocation — never assume a clean slate. Also, reuse them when context didn't change.
...
The right number of concurrent executors and worker threads within each executor depends on whether your workload is CPU-bound, memory-bound, or network-bound and goal is latency vs stability

#### Workload Type
| Workload type | Workers per executor | Reasoning |
|---|---|---|
| `cpu` | ~6 | Matches available vCPUs |
| `memory` | ~3 | Reduces heap pressure per worker. <br/> But, here we are leaving money on the table! |
| `network` | ~24 | I/O waits allow high concurrency |

#### Optimization Goal

|Optimization Goal|Target Executor Count|Strategy & Rationale|
|---|---|---|
|`Latency`|Up to 40|Run more executors in parallel to maximize speed.|
|`Stability`| Up to 10|Run fewer executors to minimize resource contention and reduce the risk of retry storms.|

The system can auto-scale from 1 to 40 executors; the upper bound (40) is driven by Step Functions parallel maps and parameter store throughput limits.
...
Even with a well-designed state machine, there are hard platform limits that constrain how large a single job can grow and how fast it can run:

### Lambda

* **Memory/Time Limits**: Max size of 10GB and 15 mins runtime. Any task that cannot be performed within these memory and time limits cannot be supported. Tasks need to be broken down into manageable chunks to overcome these limitations.

### Step Functions

* **Execution History**: Each state machine execution is capped at 25,000 events. A job with many stages and large fan-outs can approach this limit surprisingly quickly. If a single pipeline is long-running with many Driver/Executor cycles, consider splitting it into chained executions.
* **Max Runtime**: A single execution can run for up to one year — so the overall pipeline lifetime is not the concern. The event cap is.
* **Parallel Map Concurrency**: The default Inline Map state is capped at 40 concurrent executions, but you can overcome this and scale up to 10,000 parallel executions by upgrading to the Distributed Map state.

### Parameter Store

* **Throughput Limits**: The Driver and Executors use AWS SSM Parameter Store for loading credentials. Parameter Store has throughput limits on API calls (standard tier: 40 TPS; higher-throughput tier: 10,000 TPS with additional cost). At 40 concurrent executors, each making multiple reads per invocation, this can become a bottleneck. Plan ahead by batching reads and caching parameter values within a Lambda instance.
...
- **Multi-concurrent invocations**: A single execution environment can handle multiple invocations simultaneously, which increases executor throughput for IO-heavy stages without needing to spin up additional instances. But code need to support parallel invocations.
...
> Essentially, this allows the architecture to function as a highly scalable distributed system running on simple, cost-efficient EC2 instances, completely bypassing the complexity and maintenance of an EKS setup.

**Go is not yet supported as of writing this article.** Once Go support is available, this architecture becomes an even stronger candidate for high-throughput data pipelines written in Go.
