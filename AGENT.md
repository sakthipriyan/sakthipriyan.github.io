...
- **Agents hand off; they never open or merge a PR.** This holds in both repositories (this site and hugo-continuum), and even when a task seems to call for it. An agent's work ends at:
  1. one commit on its own branch, with the attribution trailer;
  2. `git push -u origin <branch>`;
  3. `gh pr create --web`, with `--title` and `--body` filled in, which opens GitHub's compare page in the owner's browser.
  The owner reviews the change there, creates the PR and merges it. Never run `gh pr create` without `--web`, `gh pr merge`, or the GitHub API to do either. Tagging a hugo-continuum release is the owner's step too.
...
- **Merging:** The owner merges, after the PR's GitHub checks pass, with a **Squash Merge** to keep the `main` history clean.
